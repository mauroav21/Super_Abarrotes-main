 ...
 * ADAPTADO para tener Bloqueo de Scroll, Centrado Forzado y Spinner de Carga,
 * imitando la funcionalidad del ModificacionProveedoresModal.
 */
...
// Ícono X para cerrar (Necesario para el header)
const X = (props) => (
...
function ModificacionUsuariosModal({ closeModal, usuario }) {
    ...
    // ----------------------------------------------------
    // 3. BLOQUEO DE SCROLL Y CLIC EN EL FONDO (NUEVA LÓGICA)
    // ----------------------------------------------------
    useEffect(() => {
    ...
    useEffect(() => {
        if (usuario) {
            ...
            axios.get(`http://localhost:8081/GetUserData/${usuario}`)
                .then(res => {
                    // Verificación básica de datos, asumiendo la estructura: { usuario: 'u1', Nombre: 'N', rol: 'A' }
                    if (res.data) {
                        setValues({
                ...
                .catch((error) => {
                    ...
                    toast.error('Error cargando los datos del usuario. Por favor, revise la consola del servidor.');
                })
                .finally(() => {
                    setLoading(false);
                });
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if (res.status === 200) {
                ...
                // Usamos setTimeout para que el toast sea visible antes de cerrar
    ...
    const handleChange = (e) => {
        ...
        // Truncamiento de valores (igual que en el original)
        const maxLengths = {
    ...
    // 7. RENDERIZADO
    // ----------------------------------------------------
    if (loading && !values.usuario) { // Muestra carga inicial
        return (
    ...
    return (
        <>
            <Toaster />
            {/* Fondo Oscuro y Posicionamiento Forzado (CSS Directo) */}
            <div
            ...
            >
                ...
                <div
                    ...
                    style={{ width: '900px', maxWidth: '90vw' }} // Ajuste el ancho para que sea más grande
                ...
                >
                    ...
                    <div className="p-4 sm:p-6 bg-indigo-700 rounded-t-xl flex items-center justify-between">
                        ...
                        <button
                            ...
                            disabled={loading} // Deshabilita durante la carga/envío
                        >
                            <X className="w-6 h-6" />
                        </button>
                    </div>
                    ...
                    <div className="p-4 sm:p-6">
                        <form onSubmit={handleSubmit} className='space-y-4'>
                            ...
                            <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                                <InputGroup label="Nombre" name="nombre" value={values.nombre} onChange={handleChange} onKeyDown={handleKeyDown} required={true} />
                                <InputGroup label="Apellido Paterno" name="apellido_paterno" value={values.apellido_paterno} onChange={handleChange} onKeyDown={handleKeyDown} required={true} />
                            </div>

                            {/* Fila 2: Apellido Materno y Usuario a Modificar */}
                            <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                                <InputGroup label="Apellido Materno" name="apellido_materno" value={values.apellido_materno} onChange={handleChange} onKeyDown={handleKeyDown} required={true} />
                            ...
                            <div className='grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-gray-200'>
                                <InputGroup label="Nueva Contraseña (Dejar vacío para no cambiar)" name="contrasena" type="password" value={values.contrasena} onChange={handleChange} maxLength="35" />
                                <InputGroup label="Confirmar Contraseña" name="confirmar_contrasena" type="password" value={values.confirmar_contrasena} onChange={handleChange} maxLength="35" />
                            </div>
