...
    if (codigo) {
      axios
        .get(`http://localhost:8081/GetProveedor/${codigo}`) // Ya corregido a /GetProveedor
        .then((res) => {
          // Aseguramos que el status HTTP sea 200 y que la respuesta tenga la estructura esperada
          if (res.status === 200 && res.data && res.data.Status === 'Exito' && res.data.Proveedor) {
            const data = res.data.Proveedor; // <--- CORRECCIÓN CLAVE: Acceder a 'Proveedor'
            setValues({
              // CORRECCIÓN CLAVE: Usar || '' para evitar undefined/null y el error de React
              codigo: data.codigo || '',
              nombre: data.nombre || '',
...
            });
          } else {
            // Manejo de error si el servidor devuelve Status: "Error" o datos inesperados
...
    // La ruta para modificar proveedor es /update_proveedor o /modificarProveedor
    // Usaré /modificarProveedor basado en tu código de backend
    const url_update = 'http://localhost:8081/modificarProveedor'; // <--- VERIFICAR ESTA RUTA EN EL SERVIDOR

    // Validación de campos a modificar (Teléfono y Correo)
    
...
    try {
      // Reemplacé la URL de actualización con la que parece ser la correcta según tus rutas de servidor
      // Si en tu servidor usas 'update_proveedor', puedes revertir el cambio de url_update
      const res = await axios.post(url_update, values); 
        setLoading(false);
